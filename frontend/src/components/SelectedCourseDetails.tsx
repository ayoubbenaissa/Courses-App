...
export const SelectedCourseDetails = ({
  selectedCourse,
  closeModal,
}: {
  selectedCourse: Course;
  closeModal: (e: MouseClickEvent) => void;
}) => {
  ...
  return (
    <div className="course-details_container" onClick={(e) => closeModal(e)}>
      <div
        className="course-details_modal"
      ...
      >
        ...
        <p>Members:</p>
        {selectedCourse.members.map((member, index) => (
          <div key={index} className="course-details_member">
            {member}
          </div>
        ))}
