...
export const fetchCourses = async (): Promise<Course[]> => {
  const courses = await makeApiCall(`http://localhost:5000/courses`, {
    method: "GET",
  });
  return courses.json();
};
...
export async function deleteCourse(courseId: string) {
  await makeApiCall(`http://localhost:5000/courses/${courseId}`, {
    method: "DELETE",
  });
}
...
export async function updateCourse(
  courseId: string,
  courseToUpdate: Omit<Course, "_id">,
): Promise<Course> {
  const updatedCourse = await makeApiCall(
